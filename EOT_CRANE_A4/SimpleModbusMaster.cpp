...
// This is limited to the Serial2 buffer of 128 bytes
...
void check_packet_status()
{
  ...
  if (packet->retries == retry_count)
    ...
    Serial.println("Total Errors : " +String(packet->total_errors));
    
    if(packet->timeout)                    Serial.println("packet->timeout");
    if(packet->incorrect_id_returned)      Serial.println("packet->incorrect_id_returned");
    if(packet->incorrect_function_returned)Serial.println("packet->incorrect_function_returned");
    if(packet->incorrect_bytes_returned)   Serial.println("packet->incorrect_bytes_returned");    
    if(packet->checksum_failed)            Serial.println("packet->checksum_failed");    
    if(packet->buffer_errors)              Serial.println("packet->buffer_errors");    
    if(packet->illegal_function)           Serial.println("packet->illegal_function");    
    if(packet->illegal_data_address)       Serial.println("packet->illegal_data_address");    
    if(packet->illegal_data_value)         Serial.println("packet->illegal_data_value");
	}
}
...
// get the Serial2 data from the buffer
unsigned char getData()
{
  ...
  {
		// The maximum number of bytes is limited to the Serial2 buffer size of 128 bytes
		// If more bytes is received than the BUFFER_SIZE the overflow flag will be set and the 
		// Serial2 buffer will be red untill all the data is cleared from the receive buffer,
  ...
  // If the buffer was partialy filled clear the buffer.
	// The maximum number of bytes in a modbus packet is 256 bytes.
	// The Serial2 buffer limits this to 128 bytes.
...
//		Serial.print(frame[i],DEC);Serial.print(' ');
	}
	Serial2.flush();
